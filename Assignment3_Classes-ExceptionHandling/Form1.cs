namespace Assignment3_Classes_ExceptionHandling
{
    public partial class Window : Form
    {
        ...
        public Window()
        {
            ...
            { TransmissionCheck, EngineCheck, AirCheck, YearComboBox, MakeComboBox, FirstNameRTB, LastNameRTB, ColourRTB, PhoneRTB, ModelRTB, CostRTB };
        }
        ...
        */
        ...
        /*
         * This event handler updates the current selected row where it first checks if a row is selected then overwrites the row selected with the validated 
         * data that the user inputs before clicking the button. This is where I learned about using MessageBox buttons for the first time where if the user 
         * does not select a row, then it displays an error message with one button and does nothing.
         */
        private void UpdateButton_Click(object sender, EventArgs e)
        {
            // Validation
            int selectedCount = DataListView.SelectedItems.Count;
            if (selectedCount == 0) { MessageBox.Show("Please select a row to update.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); }
            else if (selectedCount > 1) { MessageBox.Show("Please select only one row to update.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error); }
            else
            {
                ...
                if (MakeComboIndex != -1) { MakeComboBox.SelectedIndex = MakeComboIndex; }
                else { service.Make = MakeComboBox.Text; }
                ...
                ListViewItem selectedItem = DataListView.SelectedItems[0];
                selectedItem.SubItems[1].Text = service.calendar.ToString();
                selectedItem.SubItems[2].Text = service.firstName;
                selectedItem.SubItems[3].Text = service.lastName;
                selectedItem.SubItems[4].Text = service.phoneNumber;
                selectedItem.SubItems[5].Text = service.Make;
                selectedItem.SubItems[6].Text = service.Model;
                selectedItem.SubItems[7].Text = service.Year.ToString();
                selectedItem.SubItems[8].Text = service.Colour;
                selectedItem.SubItems[9].Text = service.Price.ToString();
                DataListView.Refresh();
            }
        }

        /* 
         * This is a helper method for the add and overwrite buttons where it helps declare and organize the user inputs to the table using the service class
         * before the true appending of the information with the add button.
        */
